...
namespace Editor
{
    public class ReplaceMaterialsInPrefabs : EditorWindow
    {
        ...
        [MenuItem("Tools/BatchOperation/Replace MapTypeToPureMaterials In Prefabs")]
        public static void ShowWindow()
        {
            GetWindow<ReplaceMaterialsInPrefabs>("Replace MapTypeToPureMaterials");
        }

        private void OnGUI()
        {
            GUILayout.Label("Replace MapTypeToPureMaterials In Prefabs", EditorStyles.boldLabel);

            newMaterial = (Material)EditorGUILayout.ObjectField("New Material", newMaterial, typeof(Material), false);
            folderPath = EditorGUILayout.TextField("Folder Path", folderPath);

            if (GUILayout.Button("Replace MapTypeToPureMaterials"))
            {
                if (newMaterial == null)
                {
