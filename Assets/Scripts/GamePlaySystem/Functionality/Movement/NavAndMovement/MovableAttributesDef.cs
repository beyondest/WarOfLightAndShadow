...
{
    /*
    public class MovableAttributesDef : MonoBehaviour
    {
        ...
        private class Baker : Baker<MovableAttributesDef>
        {
            public override void Bake(MovableAttributesDef def)
            {
                var physicsShape = def.GetComponent<PhysicsShapeAuthoring>();
                if (!def.TryGetComponent(out NavMeshAgent agent))
                {
                ...
                AddComponent(entity, new NavAgentComponent
                {
                    ...
                    CalculateInterval = def.calculateInterval,
                    Extents = float3.zero,
