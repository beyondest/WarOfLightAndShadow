...
//                          .Query<RefRW<NavAgentComponent>, RefRW<LocalTransform>>().WithEntityAccess())
//             {
//                 DynamicBuffer<WaypointBuffer> waypointBuffer = state.EntityManager.GetBuffer<WaypointBuffer>(entity);
//
//                 // Time to recalculate the path for this specific entity
...
//         {
//             // Check if is already reaching the target
//             if (math.distance(transform.ValueRO.Position, waypointBuffer[navAgent.ValueRO.currentWaypoint].wayPoint) <
//                 0.4f)
//             {
...
//                 math.normalize(direciton) * SystemAPI.Time.DeltaTime * navAgent.ValueRO.moveSpeed;
//         }
//
//
//         [BurstCompile]
...
//             ref SystemState state)
//         {
//             // Original code is Allocator.TempJob
...
//             float3 toPosition = state.EntityManager.GetComponentData<LocalTransform>(navAgent.ValueRO.targetEntity)
//                 .Position;
...
//
//                         NativeArray<NavMeshLocation> result =
